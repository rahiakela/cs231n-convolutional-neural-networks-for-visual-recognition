...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        numerator = np.exp(scores[range(N), y])
        denominator = np.sum(np.exp(scores), axis=1)

        mask = (np.exp(A2) / denominator.reshape(scores.shape[0], 1))
        mask[range(N), y] = - (denominator - numerator) / denominator
        mask /= N

        grads['W2'] = A1.T.dot(mask) + 2 * reg * W2
        grads['b2'] = np.sum(mask, axis=0)
